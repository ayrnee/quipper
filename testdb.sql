DROP TABLE USERS CASCADE CONSTRAINTS;
DROP TABLE FOLLOWS CASCADE CONSTRAINTS;
DROP TABLE TWEETS CASCADE CONSTRAINTS;
DROP TABLE MULTIMEDIA CASCADE CONSTRAINTS;
DROP TABLE COMMENTS CASCADE CONSTRAINTS;


CREATE TABLE USERS
(
  USERID VARCHAR(20) NOT NULL PRIMARY KEY,
  UNAME VARCHAR(30)
);

CREATE TABLE FOLLOWS
(
  FOLLOWER VARCHAR(20) NOT NULL,
  USERID VARCHAR(20) NOT NULL,
  PRIMARY KEY (FOLLOWER, USERID),
  FOREIGN KEY(USERID) REFERENCES USERS(USERID) ON DELETE CASCADE
);

CREATE TABLE TWEETS
(
  USERID VARCHAR(20) NOT NULL,
  TID VARCHAR(20) NOT NULL,
  PRIMARY KEY (USERID, TID),
  MSG CLOB NOT NULL, --VARCHAR(MAX)
  TSTAMP TIMESTAMP,
  FOREIGN KEY(USERID) REFERENCES USERS(USERID) ON DELETE CASCADE
);

CREATE TABLE COMMENTS
(
  USERID VARCHAR(20) NOT NULL,
  TID VARCHAR(20) NOT NULL,
  CID VARCHAR(20) NOT NULL PRIMARY KEY,
  CMT CLOB NOT NULL,
  TSTAMP TIMESTAMP,
  FOREIGN KEY (USERID, TID) REFERENCES TWEETS(USERID, TID) ON DELETE CASCADE
);

CREATE TABLE MULTIMEDIA
(
  USERID VARCHAR(20) NOT NULL,
  TID VARCHAR(20) NOT NULL,
  MEDIA BLOB,
  PRIMARY KEY (USERID, TID),
  FOREIGN KEY(USERID, TID) REFERENCES TWEETS(USERID, TID) ON DELETE CASCADE
);

INSERT INTO USERS
VALUES ('300', 'ERNEST');

INSERT INTO USERS
VALUES ('60', 'MARK');

INSERT INTO USERS
VALUES ('7', 'JIMOTHY');

INSERT INTO USERS
VALUES ('80', 'GUNDERSON');

INSERT INTO USERS
VALUES ('1', 'MO');

--ERNEST FOLLOWS GUNDERSON
INSERT INTO FOLLOWS
VALUES ('300', '80');
--MO FOLLOWS ERNEST
INSERT INTO FOLLOWS
VALUES ('1', '300');

--ERNEST FOLLOWS MO
INSERT INTO FOLLOWS
VALUES ('300', '1');

--ERNEST FOLLOWS SOMEONE THAT DOESNT EXIST
INSERT INTO FOLLOWS
VALUES ('300', '0');

--GUNDERSON FOLLOWS JIMOTHY
INSERT INTO FOLLOWS
VALUES ('80', '7');

INSERT INTO TWEETS
VALUES ('1', '1', 'GOOD MORNING',  CURRENT_TIMESTAMP);

INSERT INTO TWEETS
VALUES ('300', '2', 'MAN THAT WAS A GOOD BURGER',  CURRENT_TIMESTAMP);

INSERT INTO TWEETS
VALUES ('80', '3', 'J MY BEST FRIEND THANKS',  CURRENT_TIMESTAMP);

INSERT INTO TWEETS
VALUES ('1', '4', 'GOOD NIGHT',  CURRENT_TIMESTAMP);

INSERT INTO COMMENTS
VALUES ('1', '4', '1', 'THIS TWEET IS A VERY GOOD TWEET', CURRENT_TIMESTAMP);

-- list all users, get all tweets from a user, get all followers of a user, get all followers from a user, get all tweets from users you follow
SELECT * FROM USERS WHERE USERID = '300';

SELECT * FROM TWEETS WHERE USERID = '300';

SELECT UNAME FROM USERS WHERE USERID IN
(SELECT FOLLOWER FROM FOLLOWS WHERE USERID = '300');

SELECT UNAME FROM USERS WHERE USERID IN
(SELECT USERID FROM FOLLOWS WHERE FOLLOWER = '300');

SELECT TID, MSG FROM TWEETS WHERE USERID IN (SELECT USERID FROM FOLLOWS WHERE FOLLOWER = '300') ORDER BY TSTAMP;

SELECT CMT FROM COMMENTS WHERE TID = 4;
